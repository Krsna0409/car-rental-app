...
export function LoginScreen() {
  ...
  const handleLogin = async () => {
    ...
    // Extract name from email for personalized message
    const nameFromEmail = email.split('@')[0].replace(/[._-]/g, ' ').replace(/\b\w/g, l => l.toUpperCase())
    
    setUser({ name: nameFromEmail, email, phone: "+91 98765 43210" })
    
    // Send welcome email
    await sendWelcomeEmail(email, nameFromEmail)
  ...
  return (
    <div className="flex min-h-screen flex-col bg-background lg:justify-center lg:px-8 overflow-hidden">
      <div className="flex flex-1 flex-col items-center justify-center px-6 py-12 lg:w-full lg:max-w-md lg:mx-auto">
        <div className="w-full animate-fade-in-scale">
          <div className="flex flex-col items-center gap-3 mb-10">
            <div className="flex h-20 w-20 items-center justify-center rounded-3xl bg-primary animate-bounce-in shadow-lg hover:shadow-xl hover:scale-110 transition-all duration-300 ease-in-out cursor-pointer">
              <Car className="h-10 w-10 text-primary-foreground animate-float" />
            </div>
            <div className="text-center animate-slide-in-up">
              <h1 className="text-4xl font-bold text-primary mb-2">Rev Your Engines!</h1>
          ...
          <div className="space-y-5">
            ...
            <Button 
              onClick={handleLogin} 
              disabled={isLoading}
              className="w-full h-12 text-base font-bold mt-6 opacity-0 animate-slide-in-up bg-primary hover:bg-primary/90 text-primary-foreground relative overflow-hidden group shadow-lg hover:shadow-xl hover:-translate-y-0.5 active:translate-y-0 transition-all duration-300 ease-in-out"
